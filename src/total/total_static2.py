    ...
    def delete_vector_inside_obstacle(self, detecting_points):
        ...
        while (obstacle_number) != len(static_OB_data):     
            ...
            for i in range(self.angle_number+1): 
                if  so.staticOB_cal(pA[0], pA[1], detecting_points[i][0], detecting_points[i][1], obstacle_point_x[0], obstacle_point_y[0], obstacle_point_x[1], obstacle_point_y[1], 3)== False:
                    self.non_cross_vector.append(detecting_points[i]) # 여기가 크로스 여부 확인하여 어떻게 할지 하는 부분
                else: # True 일때 
                    pass
                
            if len(self.non_cross_vector) == 0:
                self.non_cross_vector.append(detecting_points[self.angle_number])
                self.non_cross_vector.append(detecting_points[self.angle_number-1])
